...
Observador (opción A) + Grover.
Estructura base: 3 planos de 4 qubits, wt==2 por plano, 3 ejes alineados.

± global (lo más fiel a tu fórmula):
  Definimos el signo global como el PRODUCTO de los 12 signos s_i ∈ {-1,+1}.
  Mapeo: bit=1 -> +1, bit=0 -> -1.
  Entonces:
      S = Π_i s_i = (-1)^(#ceros) = (-1)^(12 - popcount)
  Como 12 es par, esto equivale a:
      S = +1  <=> popcount total es PAR
      S = -1  <=> popcount total es IMPAR

IMPORTANTE:
- Con wt==2 por plano, el popcount total = 6 SIEMPRE, por lo que S=+1 siempre.
- Es decir: para la estructura actual, la rama '-' quedará vacía (M=0).
Esto no es un bug: es una consecuencia matemática de exigir equilibrio wt=2 en 3 planos.

Notas:
- Marco físico: s_phys = s_measured[::-1] (según tus pruebas).
...
# ---------------------------
# SIGN = +1 -> aceptar S=+1 (producto de 12 signos)
# SIGN = -1 -> aceptar S=-1
SIGN = +1
...
# ---------------------------
# Signo global por producto de 12 signos
# ---------------------------

def popcount(s_phys: str) -> int:
    return sum(1 for ch in s_phys if ch == "1")


def global_sign_bit(s_phys: str) -> int:
    return 1 if (popcount(s_phys) % 2 == 0) else 0
...
def coherent_string_phys(s_phys: str) -> bool:
    ...
    sbit = global_sign_bit(s_phys)  # 1 => +, 0 => -
    sign_ok = (sbit == 1) if SIGN == +1 else (sbit == 0)

    return w_ok and axis0_ok and axis1_ok and axis2_ok and sign_ok
...
# ---------------------------
# Circuito: oracle incluye signo global (paridad popcount)
# ---------------------------
...
def build_circuit(iterations: int) -> QuantumCircuit:
    ...
    t = QuantumRegister(7, "t")   # t[0..5] ejes; t[6] paridad popcount
    ...
    for _ in range(iterations):
        ...
        # paridad popcount en t[6]
        for i in range(12):
            qc.cx(q[i], t[6])

        if SIGN == +1:
            qc.x(t[6])  # control=1 <=> popcount PAR

        qc.mcx([w0[0], w1[0], w2[0], eq0[0], eq1[0], eq2[0], t[6]], ph[0])
...
def run(shots: int = 4096, iterations: int | None = None, opt_level: int = 1) -> None:
    ...
    print(f"SIGN={sign_label} (12-sign product; bits: popcount {'PAR' if SIGN==+1 else 'IMPAR'})")
    ...
    for s, c in top10:
        ...
        print(s, c, "phys=", s_phys, "ok=", ok,
              "popcount=", popcount(s_phys),
              "Sbit=", global_sign_bit(s_phys),
