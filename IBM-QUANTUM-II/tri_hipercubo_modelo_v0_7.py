...
Versión 0.7 del modelo tri-hipercúbico dual.

Qué añade respecto a v0.6:
1. Explicación algebraica explícita de por qué el subespacio coherente tiene 256 estados:
   - dimensión total binaria = 12
   - restricciones = 4
   - dimensión efectiva = 8
   - tamaño del subespacio = 2^8 = 256
2. Sección de geometría de triadas:
   - cada triada (A_i, B_i, C_i) admite 4 estados coherentes
   - esos 4 estados forman un tetraedro lógico
   - la estructura global puede leerse como (tetraedro)^4 = 4^4 = 256
3. Sección de interpretación de la fórmula estructural:
   ({-1,+1}^4)^3
4. Sección explícita del grupo de simetría del modelo:
   S4 × S3, con tamaños 24, 6 y 144.
5. Observación geométrica automática del residuo 6.
"""
...
def is_coherent(state: Sequence[int]) -> bool:
    """Comprueba si un estado cumple la restricción C_i = A_i * B_i."""
    a = state[:4]
    b = state[4:8]
    c = state[8:12]
...
def graph_metrics(n_dims: int = 12) -> Dict[str, int]:
    """Calcula la doble métrica del modelo.
...
def structural_block_matrix() -> np.ndarray:
    """Matriz estructural no trivial del modelo.

    - conexión interna dentro de cada bloque A, B, C: peso 1.0 (sin diagonal)
    - conexión A_i <-> B_i: peso 2.0
    - conexión A_i <-> C_i y B_i <-> C_i: peso 2.0
    """
...
def entropy_from_probs(probs: Iterable[float]) -> float:
    """Entropía en bits a partir de una lista de probabilidades."""
...
def mutual_information_matrix(states: Sequence[Sequence[int]]) -> np.ndarray:
    """Matriz 12x12 de información mutua entre A1..C4.

    La correlación lineal y la MI pairwise pueden resultar casi triviales,
    porque la restricción C_i = A_i * B_i es una dependencia conjunta de orden superior.
    """
...
def triad_information_report(states: Sequence[Sequence[int]]) -> List[Dict[str, float]]:
    """Analiza cada triada (A_i, B_i, C_i)."""
    X = np.array(states, dtype=int)
...
def eigensystem(matrix: np.ndarray) -> Dict[str, object]:
    """Calcula el espectro de una matriz."""
...
def global_sign_flip(state: Sequence[int]) -> Tuple[int, ...]:
    """Invierte signo de los 12 ejes. Se espera que no preserve el modelo."""
...
def structural_automorphisms(structural_matrix: np.ndarray) -> Dict[str, object]:
    """Calcula el grupo de automorfismos de la matriz estructural."""
    G = nx.Graph()
    ...
    for mapping in gm.isomorphisms_iter():
        count += 1
        if len(examples) < 6:
            examples.append({LABELS[k]: LABELS[v] for k, v in sorted(mapping.items())})
...
def invariant_report(states: Sequence[Sequence[int]]) -> Dict[str, object]:
    """Evalúa invariantes exactos inducidos por C_i = A_i*B_i."""
    sum_abc, prod_abc = [], []
    all_axis_ok = True
    for s in states:
        a = s[:4]
...
def state_distance_report(states: Sequence[Sequence[int]]) -> Dict[str, object]:
    """Analiza la geometría del conjunto de 256 estados coherentes."""
...
def dimension_profile_distance(states: Sequence[Sequence[int]]) -> np.ndarray:
    """Distancia de Hamming entre dimensiones vistas como perfiles sobre los estados."""
    X = np.array(states, dtype=int).T
    n = X.shape[0]
    D = np.zeros((n, n), dtype=float)
    for i in range(n):
        diff = (X[i] != X).sum(axis=1)
...
def matrix_to_list(M: np.ndarray) -> List[List[float]]:
    """Convierte una matriz numpy a lista JSON-friendly redondeada."""
    return [[float(np.round(v, 8)) for v in row] for row in M.tolist()]


def print_section(title: str) -> None:
    """Imprime un bloque visual de salida por consola."""
    print('\n' + '=' * 78)
    print(title)
    print('=' * 78)


def algebraic_interpretation(n_binary_dims: int, n_constraints: int) -> Dict[str, int]:
    """Resume la reducción algebraica del modelo.

    12 variables binarias - 4 restricciones = 8 grados efectivos.
    2^8 = 256 estados coherentes.
    """
    effective_dim = n_binary_dims - n_constraints
    return {
        'dimension_total_binaria': n_binary_dims,
        'restricciones': n_constraints,
        'dimension_efectiva': effective_dim,
        'tamano_subespacio': 2 ** effective_dim,
    }


def triad_geometry() -> Dict[str, object]:
    """Describe la geometría lógica de una triada y del producto total.

    Cada triada admite exactamente 4 estados coherentes:
    (+,+,+), (+,-,-), (-,+,-), (-,-,+)
    que pueden interpretarse como los 4 vértices de un tetraedro lógico.
    """
    triad_states = [
        (+1, +1, +1),
        (+1, -1, -1),
    ...
    return {
        'estados_posibles_por_triada': len(triad_states),
        'estados_triada': triad_states,
        'estructura_geometrica': 'tetraedro',
        'numero_de_triadas': 4,
        'estructura_total': '(tetraedro)^4',
        'espacio_coherente': len(triad_states) ** 4,
    }


def formula_interpretation() -> Dict[str, object]:
    """Conecta el programa con la fórmula estructural del modelo: ({-1,+1}^4)^3."""
    return {
        'formula_textual': '({-1,+1}^4)^3',
        'interpretacion': {
            '{-1,+1}': 'variable binaria',
            '^4': '4 ejes por bloque',
            '^3': '3 bloques (A, B, C)',
        },
        'dimensiones_totales': 4 * 3,
        'restriccion_clave': 'C_i = A_i * B_i',
        'dimension_efectiva': 12 - 4,
        'estados_coherentes': 2 ** 8,
    }


def symmetry_group_interpretation(auto_size: int) -> Dict[str, object]:
    """Explicita el grupo S4 × S3 y su tamaño."""
    s4 = 24
    s3 = 6
    return {
        'grupo': 'S4 × S3',
        ...
        'tamano_S3': s3,
        'grupo_total': s4 * s3,
        'tamano_automorfismos_detectado': auto_size,
        'coincidencia_con_automorfismos': auto_size == s4 * s3,
    }


def residual_geometric_observation(residual: int) -> Dict[str, object]:
    """Observación interpretativa sobre el residuo 6."""
    return {
        'residuo': residual,
        'nota': 'El residuo 6 coincide con los 6 ejes espaciales duales (+x, -x, +y, -y, +z, -z).',
        'tipo': 'observacion_geometrica',
        'estatus': 'No se presenta como resultado formal del modelo.',
    }


def render_html(report: Dict[str, object], path: Path) -> None:
    """Genera un HTML-resumen legible y sin dependencias externas."""
    combos = report['metrica_combinatoria']
    invariants = report['invariantes']
    sym = report['simetrias']
    auto = report['automorfismos']
    triads_info = report['triadas_informacion']
    triad_geo = report['geometria_triadas']
    alg = report['interpretacion_algebraica']
    formula = report['interpretacion_formula']
    group = report['grupo_simetria_modelo']
    residue = report['observacion_geometrica_residuo']
...
<head>
<meta charset=\"utf-8\">
<title>Tri-hipercubo dual v0.7</title>
<style>
...
<body>
<h1>Modelo tri-hipercúbico dual · v0.7</h1>
<p class=\"small\">Versión explicativa: combina cálculo, interpretación algebraica y lectura geométrica del modelo.</p>

<div class=\"card\">
<h2>Interpretación algebraica</h2>
<ul>
<li>Dimensión total binaria: <b>{alg['dimension_total_binaria']}</b></li>
<li>Restricciones: <b>{alg['restricciones']}</b></li>
<li>Dimensión efectiva: <b>{alg['dimension_efectiva']}</b></li>
<li>Tamaño del subespacio: <b>{alg['tamano_subespacio']}</b></li>
</ul>
</div>

<div class=\"card\">
<h2>Geometría de triadas</h2>
<ul>
<li>Estados posibles por triada: <b>{triad_geo['estados_posibles_por_triada']}</b></li>
<li>Estructura geométrica: <b>{triad_geo['estructura_geometrica']}</b></li>
<li>Número de triadas: <b>{triad_geo['numero_de_triadas']}</b></li>
<li>Estructura total: <b>{triad_geo['estructura_total']}</b></li>
<li>Espacio coherente: <b>{triad_geo['espacio_coherente']}</b></li>
</ul>
<p><code>{triad_geo['estados_triada']}</code></p>
</div>

<div class=\"card\">
<h2>Interpretación de la fórmula</h2>
<ul>
<li>Fórmula: <code>{formula['formula_textual']}</code></li>
<li>{{-1,+1}} = {formula['interpretacion']['{-1,+1}']}</li>
<li>^4 = {formula['interpretacion']['^4']}</li>
<li>^3 = {formula['interpretacion']['^3']}</li>
<li>Dimensiones totales: <b>{formula['dimensiones_totales']}</b></li>
<li>Restricción clave: <code>{formula['restriccion_clave']}</code></li>
<li>Dimensión efectiva: <b>{formula['dimension_efectiva']}</b></li>
<li>Estados coherentes: <b>{formula['estados_coherentes']}</b></li>
</ul>
</div>

<div class=\"card\">
<h2>Grupo de simetría del modelo</h2>
<ul>
<li>Grupo: <b>{group['grupo']}</b></li>
<li>Tamaño grupo S4: <b>{group['tamano_S4']}</b></li>
<li>Tamaño grupo S3: <b>{group['tamano_S3']}</b></li>
<li>Grupo total: <b>{group['grupo_total']}</b></li>
<li>Automorfismos detectados: <b>{group['tamano_automorfismos_detectado']}</b></li>
</ul>
</div>

<div class=\"card\">
<h2>Observación geométrica del residuo</h2>
<ul>
<li>Residuo: <b>{residue['residuo']}</b></li>
<li>{residue['nota']}</li>
<li class=\"small\">{residue['estatus']}</li>
</ul>
</div>
...
<p><b>Espectro MI:</b> {spec_mi['eigenvalores']}</p>
<p><b>Espectro estructural:</b> {spec_struct['eigenvalores']}</p>
<p class=\"small\">Si la información mutua pairwise sale casi trivial, la dependencia fuerte del modelo es triádica.</p>
</div>
...
<div class=\"card\">
<h2>Automorfismos y simetrías naturales</h2>
<ul>
...
def main() -> None:
    """Orquesta todo el análisis del modelo y genera consola, JSON y HTML."""
    H = hypercube_4d()
    ...
    embed_state_hamming = classical_mds(state_dist['matrix'], [f's{i}' for i in range(len(states))], n_components=3)
    ...
    triads_info = triad_information_report(states)
    sym = symmetry_report(states)
    auto = structural_automorphisms(S)
    inv = invariant_report(states)

    alg = algebraic_interpretation(n_binary_dims=12, n_constraints=4)
    triad_geo = triad_geometry()
    formula = formula_interpretation()
    group = symmetry_group_interpretation(auto['tamano_grupo_automorfismos'])
    residue = residual_geometric_observation(combos['residuo_78_menos_72'])

    report: Dict[str, object] = {
        'modelo': 'tri_hipercubo_dual_v0_7',
        'estructura': {
        ...
        'interpretacion_algebraica': alg,
        'geometria_triadas': triad_geo,
        'interpretacion_formula': formula,
        'grupo_simetria_modelo': group,
        'observacion_geometrica_residuo': residue,
        ...
        'triadas_informacion': triads_info,
        'simetrias': sym,
        'automorfismos': auto,
        'invariantes': inv,
        'distancias_estados': {
    ...
    json_path = OUTDIR / 'tri_hipercubo_modelo_v0_7_resultado.json'
    with json_path.open('w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)

    html_path = OUTDIR / 'tri_hipercubo_modelo_v0_7_explorador.html'
    render_html(report, html_path)

    print_section('MODELO TRI-HIPERCÚBICO DUAL · VERSIÓN 0.7')
    ...
    print_section('INTERPRETACIÓN ALGEBRAICA')
    print(f'Dimensión total binaria              : {alg["dimension_total_binaria"]}')
    print(f'Restricciones                        : {alg["restricciones"]}')
    print(f'Dimensión efectiva                   : {alg["dimension_efectiva"]}')
    print(f'Tamaño del subespacio                : 2^{alg["dimension_efectiva"]} = {alg["tamano_subespacio"]}')

    print_section('GEOMETRÍA DE TRIADAS')
    print(f'Estados posibles por triada          : {triad_geo["estados_posibles_por_triada"]}')
    print(f'Estructura geométrica                : {triad_geo["estructura_geometrica"]}')
    print(f'Número de triadas                    : {triad_geo["numero_de_triadas"]}')
    print(f'Estructura total                     : {triad_geo["estructura_total"]}')
    print(f'Espacio coherente                    : 4^{triad_geo["numero_de_triadas"]} = {triad_geo["espacio_coherente"]}')
    print(f'Estados por triada                   : {triad_geo["estados_triada"]}')

    print_section('INTERPRETACIÓN DE LA FÓRMULA')
    print(f'Fórmula estructural                  : {formula["formula_textual"]}')
    print(f'{{-1,+1}}                            : {formula["interpretacion"]["{-1,+1}"]}')
    print(f'^4                                   : {formula["interpretacion"]["^4"]}')
    print(f'^3                                   : {formula["interpretacion"]["^3"]}')
    print(f'Dimensiones totales                  : {formula["dimensiones_totales"]}')
    print(f'Restricción                          : {formula["restriccion_clave"]}')
    print(f'Dimensión efectiva                   : {formula["dimension_efectiva"]}')
    print(f'Estados coherentes                   : {formula["estados_coherentes"]}')
    ...
    print_section('OBSERVACIÓN GEOMÉTRICA DEL RESIDUO')
    print(f'Residuo                              : {residue["residuo"]}')
    print(f'Nota                                 : {residue["nota"]}')
    print(f'Estatus                              : {residue["estatus"]}')
    ...
    print_section('GRUPO DE SIMETRÍA DEL MODELO')
    print(f'Grupo                                : {group["grupo"]}')
    print(f'Tamaño grupo S4                      : {group["tamano_S4"]}')
    print(f'Tamaño grupo S3                      : {group["tamano_S3"]}')
    print(f'Grupo total                          : {group["grupo_total"]}')
    print(f'Automorfismos detectados             : {group["tamano_automorfismos_detectado"]}')
    print(f'Coincidencia con el grupo esperado   : {group["coincidencia_con_automorfismos"]}')
